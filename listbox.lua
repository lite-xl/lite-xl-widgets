...
---Used to indicate on a widget.listbox.row row that the end
---of a column was reached.
---@type integer
ListBox.COLEND = 1

---Used to indicate on a widget.listbox.row row a new line
...
function ListBox:draw_row(row, x, y, only_calc)
  ...
  if not only_calc then
    if self:mouse_on_top(self.mouse.x, self.mouse.y) then
      w, h = self:draw_row(row, x, y, true)
